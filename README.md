...
以下示例面向 Ubuntu/Debian，把程序安装到 `/opt/picnest`，并使用专用系统用户运行。完整顺序为：准备环境、下载代码、构建、配置环境变量、启动 systemd、选择 Nginx 或 Caddy、启用 HTTPS、创建首位管理员。不要在 HTTPS 和反向代理完成前执行首次初始化。
...
| `PICNEST_STORAGE_SECRET` | 使用会话密钥 | 云存储、WebDAV 凭据和可查看 API 密钥的加密密钥，生产环境应独立配置并备份，投入使用后不可更换 |
| `PICNEST_API_MONTHLY_LIMIT` | `50000` | 每位用户的月度 API 密钥调用额度，达到上限后返回 `429` |
| `PICNEST_PUBLIC_URL` | 根据请求自动识别 | 对外访问根地址，例如 `https://img.example.com`；经过反向代理时建议显式配置，返回的图片直链会使用该地址 |
| `COOKIE_SECURE` | `false` | HTTPS 反代时必须设为 `true`；纯 HTTP 本地测试保持 `false` |
| `NODE_ENV` | 未设置 | 生产服务建议设为 `production` |

生产部署统一在 `/etc/picnest/picnest.env` 中设置 `PORT`。如果把 `18765` 改成其他端口，必须同时修改所选 Nginx 配置的 `proxy_pass` 或 Caddy 配置的 `reverse_proxy`；本地开发还需要同步修改 `vite.config.ts` 中的三个代理地址。
...
这里必须使用实际域名，不能保留 `img.example.com`。然后重启：
...
完成 systemd、Nginx/Caddy、DNS 和 HTTPS 配置后，再执行以下步骤：
...
每位用户可以为 PicGo、ShareX、服务器脚本等客户端分别创建多把 Bearer API 密钥，并在开发者页面查看、复制或删除。完整密钥使用 `PICNEST_STORAGE_SECRET` 加密保存；升级前已经存在的旧密钥只有哈希，仍可继续使用，但无法恢复完整内容，需要查看时应删除后重新创建。密钥管理接口只接受网页登录会话，Bearer 密钥不能读取或删除其他密钥。
...
| `PATCH` | `/api/users/:id` | 管理员 | 修改成员资料、角色、密码、配额和存储策略 |
...
| `PATCH` | `/api/settings/image-processing` | 管理员 | 修改允许上传类型与默认图片处理策略 |
| `GET` | `/api/storage/providers` | 用户 | 查看已脱敏的存储服务列表 |
| `POST` | `/api/storage/providers` | 管理员 | 添加存储配置 |
| `PATCH/DELETE` | `/api/storage/providers/:id` | 管理员 | 修改或删除存储配置 |
| `POST` | `/api/storage/providers/:id/test` | 管理员 | 检测存储写入、读取和删除能力 |
| `PATCH` | `/api/storage/providers/:id/default` | 管理员 | 检测并切换当前存储 |
