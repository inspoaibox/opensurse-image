...
try {
  ...
  await page.getByRole('heading', { name: '根目录', exact: true }).waitFor()
  await page.getByRole('button', { name: '新建文件夹' }).click()
  await page.getByLabel('文件夹名称').fill('UI 文档')
  await page.getByRole('button', { name: '创建文件夹' }).click()
  await page.getByRole('button', { name: /打开文件夹 UI 文档/ }).waitFor()
  ...
  await page.getByRole('button', { name: '文件列表视图' }).click()
  await page.reload({ waitUntil: 'networkidle' })
  await page.getByRole('heading', { name: '工作台', exact: true }).waitFor()
  await page.getByRole('button', { name: '文件库' }).click()
  await page.getByRole('heading', { name: '根目录', exact: true }).waitFor()
  const fileListButton = page.getByRole('button', { name: '文件列表视图' })
  await fileListButton.waitFor()
  assert.equal(await fileListButton.evaluate((button) => button.classList.contains('active')), true)
  await page.getByRole('button', { name: '查看文件 上线检查文件改名.pdf' }).waitFor()
